"""AP2 merchant authorization for the Business service.
...
_merchant_private_key, _merchant_public_jwk = generate_ec_key("merchant_001")
...
def sign_checkout_response(checkout_dict: dict) -> dict:
    ...
    Removes the ap2 field before canonicalizing and signing, then
    adds the signature back.
    """
    # Work on a copy to avoid mutating the original during canonicalization
...
async def verify_checkout_mandate(
    ...
    platform_jwk = keys[0]  # Use first key
    ...
    # Verify the embedded checkout matches the current session
    ...
    # Verify the embedded merchant_authorization is our own valid signature
...
async def verify_intent_mandate(
    ...
    # Get transaction amount (in cents for comparison)
    ...
    # Check key binding claims for amount and use_index
    # These are in the kb JWT which was already verified by verify_sd_jwt_kb
    # We need to re-parse to get kb claims
    ...
    if len(parts) == 2:
        ...
        if cnf_jwk:
            ...
            use_index = kb_claims.get("use_index", 0)
            ...
            # Usage tracking (by issuer JWT hash as mandate ID)
            issuer_jwt = parts[0]
            import hashlib
